...
1.  **[ ] `CoE_Implementation_Guide.md`**：
    *   `version` -> `v1.2.0`
    *   `compatible_book_range` -> `["v1.1.0", "v1.2.x"]`
2.  **[ ] `Enterprise_Context_Schema.md`**：
    *   `version` -> `v1.2.0`
3.  **[ ] `Upgrade_Proposals_v1.2.0.md`**：
    *   標註計畫已進入「實施階段」。

### B. 診斷架構升級 (Schema Update)
1.  **[ ] `Enterprise_Context_Schema.md` - 第一章重構**：
    *   移除單一的「產業樣態分類」。
    *   新增「二維權重矩陣座標」檢核項。
    *   新增「企業 AI 遺傳密碼：五大屬性」描述欄位。
2.  **[ ] `Enterprise_Context_Schema.md` - 組織評估優化**：
    *   加入「先行者密度」與「五星特質」檢核清單。

### C. 行動指南升級 (Guide Update)
1.  **[ ] `CoE_Implementation_Guide.md` - 第零階段**：
    *   插入「五大屬性座標診斷」行動項。
2.  **[ ] `CoE_Implementation_Guide.md` - Wing Group 循環**：
    *   插入「內容切片 (S-代號) 與非同步歸檔」SOP 行動項。
3.  **[ ] `CoE_Implementation_Guide.md` - 結尾補充**：
    *   新增「流程有效性自我驗證」檢查清單。

### D. 核心書籍內容增補 (Book Chapters Update)
1.  **[ ] `Comprehensive_Enterprise_GenAI_Transformation.md`** (或 `/Chapter_1_2.md`)：
    *   改寫診斷理論，引入矩陣與座標概念。
2.  **[ ] `Comprehensive_Enterprise_GenAI_Transformation.md`** (或 `/Chapter_5_3.md`)：
    *   增補「五星特質」先行者網絡定義。
3.  **[ ] 新增獨立檔案 `Metrics_Encyclopedia_Guide.md`**：
