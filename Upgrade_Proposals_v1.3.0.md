...
1.  **技能資產化 SOP**：定義如何快速將「老師傅經驗」轉化為機器可讀的工具描述與 Prompt。
2.  **自動化評測對齊 (Eval-Alignment)**：建立不依賴人為直覺的自動評分機制，讓 Agent 具備自我質檢能力。
3.  **知識工程維管**：強化跨部門知識在安全隔離下的檢索效率。
...
*   **虛擬 COE 指揮官 (Virtual COE Agent)**：作為長期技術研發項目，暫不納入 v1.x 的正式落地清單。
